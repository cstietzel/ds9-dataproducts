...
absent <- data_frame()
files <- list.files(path="~/EdDataCT", pattern = "chronicAbsenteeism1", full.names = TRUE)
for (f in files) {
  flen <- nchar(f)
...
spending <- data_frame()
files <- list.files(path="~/EdDataCT", pattern = "perPupilExpeditures", full.names = TRUE)
for (f in files) {
  flen <- nchar(f)
...
files <- list.files(path="~/EdDataCT", pattern = "enrollmentSingleYear", full.names = TRUE)
for (f in files) {
  flen <- nchar(f)
...
results <- read_xlsx(path="~/EdDataCT/nextgenresults15-16.xlsx", sheet = 3) %>%
